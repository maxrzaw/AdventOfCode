...
while (queue.length > 0) {
    ...
    while (minute < currentPosition.minute + 2) {
        SimulateMinute(minute, valley);
        log(`Simulating minute: ${minute}`);
        minute++;
    }

    // log(currentPosition, trip);

    if (trip === 2 && currentPosition.row === start.row && currentPosition.column === start.column) {
        log(`Back to start at minute: ${currentPosition.minute}`);
        start.minute = currentPosition.minute;
        trip = 3;
        queue = [start];
        seen = new Set<string>();
        continue;
    ...
    if ([1, 3].includes(trip) && currentPosition.row === end.row && currentPosition.column === end.column) {
        end.minute = currentPosition.minute;
        if (trip === 3) {
            log("reached end again");
            break;
        }
        seen = new Set<string>();
        queue = [end];
    ...
    for (let move of moves) {
        ...
        if (isSeen ||
            ...
            nextPosition.row > valley.length - 1 ||
            nextPosition.column > valley[nextPosition.row].length - 1 ||
